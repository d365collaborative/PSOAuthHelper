Describe "Get-ClientCredentialsBearerToken Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Get-ClientCredentialsBearerToken).ParameterSets.Name | Should -Be '__AllParameterSets'
		}
		
		It 'Should have the expected parameter AuthProviderUri' {
			$parameter = (Get-Command Get-ClientCredentialsBearerToken).Parameters['AuthProviderUri']
		...
		It 'Should have the expected parameter Resource' {
			$parameter = (Get-Command Get-ClientCredentialsBearerToken).Parameters['Resource']
		...
		It 'Should have the expected parameter ClientId' {
			$parameter = (Get-Command Get-ClientCredentialsBearerToken).Parameters['ClientId']
		...
		It 'Should have the expected parameter ClientSecret' {
			$parameter = (Get-Command Get-ClientCredentialsBearerToken).Parameters['ClientSecret']
		...
		It 'Should have the expected parameter Scope' {
			$parameter = (Get-Command Get-ClientCredentialsBearerToken).Parameters['Scope']
		...
		It 'Should have the expected parameter Raw' {
			$parameter = (Get-Command Get-ClientCredentialsBearerToken).Parameters['Raw']
	...
	Describe "Testing parameterset __AllParameterSets" {
		...
		__AllParameterSets -AuthProviderUri -Resource -ClientId -ClientSecret -Scope -Raw
		#>
	}
